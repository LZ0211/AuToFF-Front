<template>
  <div class="page_mol-top">
    ...
    <div style="display: flex; align-items: center;" v-if="showUpload">
      <div style="margin-right: 18px; font-size: 12px; color: #999999">{{ tips }}</div>
      <div style="
                      width: 101px;
                      height: 36px;
                      line-height: 36px;
        ...
        <a-upload name="file" accept=".mol,.pdb,.mol2,.xyz,.cif" :multiple="false" :showUploadList="false"
          style="width: 101px;" :beforeUpload="beforeUpload">
          <div
      ...
      <a-button @click="downloadOriginFile" v-if="originFileName && originFileContent"
        style="height: 36px; margin-left: 20px;" icon="download">
        源文件下载
      </a-button>
    </div>
  </div>
...
export default {
  props: {
    ...
    tips: {
      type: String,
      default:
        ".mol、.pdb、.xyz、.mol2文件上传文件大小1M以内且原子数小于1000（显示卡顿请使用更好的GPU）",
    },
  },
  data() {
    return {};
  ...
  methods: {
    beforeUpload(file) {
      ...
      if (
        ...
        this.$message.error("目前只支持.mol、.pdb、.xyz、.mol2文件上传");
        return;
      }
      // 校验文件大小
