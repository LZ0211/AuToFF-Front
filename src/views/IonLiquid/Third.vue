    ...
    <div class="third-wrapper-content">
      ...
      <div style="display: flex; flex: 1; flex-flow: column; margin-left: 24px">
        ...
        <div class="list-wrapper-btns">
          <div style="margin-right: 20px">请选择计算软件</div>
...
export default {
  ...
  created() {
    ...
    const pFirst = this.initBackForceAtomtypes();
    pFirst.then((res) => {
      const p4 = this.initParams(res);
        ...
        .then(([data4]) => {
          ...
          chargeData = data4.atoms.map((item, index) => {
            return {
              ...
              charge: item[6],
              noEdit:
                this.waterModelForces &&
                this.waterModelForces.includes(item[2]),
            };
          });
  ...
  methods: {
    // 获取备选力场的atomtype
    initBackForceAtomtypes() {
      let forcefield = "";
      forcefield = `${this.backForce}`;
    ...
    initParams(backData) {
      // console.log("备选力场对应的atomtype", backData);
      return new Promise((resolve, reject) => {
        // 精确搜索 or 模糊搜索，传过去所有的
    ...
    // 选择那一个label
    chooseWhichLabel(e) {
      this.whichLabel = e.target.value;
      if (e.target.value === "none") {
        this.hideAllLabels();
      } else if (e.target.value === "charge") {
        this.hideAllLabels();
        this.$refs["molecule-third"].showLabels();
      } else if (e.target.value === "atom") {
        this.hideAllLabels();
        this.$refs["molecule-third"].showAtomLabels();
      } else if (e.target.value === "id") {
        this.hideAllLabels();
        this.$refs["molecule-third"].showIdLabels();
      }
    },
    hideAllLabels() {
      this.$refs["molecule-third"].hideLabels();
      this.$refs["molecule-third"].hideAtomLabels();
      this.$refs["molecule-third"].hideIdLabels();
    },
    // 返回上一步
    goBack() {
      Spin.show();
    ...
    downloadFile(formData) {
      ...
      // console.log("atomtypeTableData", this.atomtypeTableData);
      ...
      // console.log("anglesData", anglesData);
      ...
      // console.log("dihedralsData", dihedralsData);
      ...
      // let anglesData = [];
      // this.anglesColumns.forEach((item, index) => {
      //   let d = this.$refs[`anglesTable${index}`].data;
      //   anglesData = anglesData.concat(d);
      // });

      // let dihedralsData = [];
      // this.dihedralsColumns.forEach((item, index) => {
      //   let d = this.$refs[`dihedralsTable${index}`].data;
      //   dihedralsData = dihedralsData.concat(d);
      // });
      // let impropersData = [];
      // this.impropersColumns.forEach((item, index) => {
      //   let d = this.$refs[`impropersTable${index}`].data;
      //   impropersData = impropersData.concat(d);
      // });
      // const pairsData = this.$refs["pairsTable"].data.map((item, index) => {
      //   return [item.first, item.second, item.third];
      // });
      ...
      return new Promise((resolve, reject) => {
        request.postFile(api[this.computer], params).then((res) => {
