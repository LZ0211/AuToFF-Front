    ...
    <div class="third-wrapper-content">
      ...
      <div style="display: flex; flex: 1; flex-flow: column; margin-left: 24px">
        ...
        <div class="list-wrapper-btns">
          ...
          >
            <el-option
              v-for="(item, index) in supportedComputers"
              :value="item"
              :key="index"
              >{{ item }}</el-option
            >
...
export default {
  ...
  data() {
    return {
      ...
      computer: null,
      // 支持的软件格式下载，从后台接口里面取
      supportedComputers: [], // params的atoms第三列在其中的话就只显示两个计算软件，不在的话就显示全。
    };
  },
  components: {
    MoleculeUltimate,
  ...
  created() {
    ...
    Promise.all([p4, p5])
      .then(([data4, data5]) => {
        ...
        // 更新计算软件的下拉选项
        let computerKey = data4.atoms[0][2];
        // console.log("computerKey", computerKey);
        if (data5.includes(computerKey)) {
          this.supportedComputers = ["lammps", "moltemplate"];
          this.computer = "lammps";
        } else {
  ...
  methods: {
    ...
    initComputers() {
      return new Promise((resolve, reject) => {
        request.get(`${api.computerList}`).then((res = {}) => {
          if (res) {
            resolve(res.data);
          }
        });
